...
Native Sonos grouping only accepts Sonos entities. Music Assistant can group a broader set of speakers, but only players exposed as compatible Music Assistant players can be grouped together. Some TVs, Cast endpoints, displays, or restored/unavailable players can play media but cannot join a synchronized group. Do not imply universal grouping.

If the user mixes a native Sonos entity with Music Assistant/non-Sonos entities, resolve selected rooms to matching Music Assistant players before calling `media_player.join`. Matching is by `<entity>_2` first and then exact friendly name. If the main speaker has no Music Assistant match, show a card error and do not call native Sonos join.
